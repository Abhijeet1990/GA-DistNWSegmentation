...
gp_problem = GraphPartition(G, adj_matrix=Adj, partition_size_upper=max_no_subgraphs,partition_size_lower=min_no_subgraphs,xcoord=np.array(xcoords), ycoord=np.array(ycoords))
start = time.time()
...
if res_gp_mo.X is not None:
    results[str(max_no_subgraphs)+'_'+str(min_no_subgraphs)] = res_gp_mo.X
    results['score'] = res_gp_mo.F
    scipy.io.savemat('new_problem_ieee13bus_der_result_20cp_50ps_80os_'+str(max_no_subgraphs)+'_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.mat',results)
...
plt.yscale("log")
plt.savefig('new_problem_ieee13bus_der_result_20cp_50ps_80os_'+str(max_no_subgraphs)+'_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.png',dpi=75)
plt.show()
