...
m = res_score.shape[0]
ref_points = np.zeros((m,3))
for i in range(m):
    ref_points[i]= res_score[i]
#ref_points = np.array([[474.06808, 28821.5, 1683.0 ],[413.166, 28427.875, 1740.0],[471.94, 27797.625, 1806.0]])



# create the algorithm object
# nsga3_alg = NSGA3(pop_size=92,
#                   ref_dirs=ref_dirs)

# Get Algorithm
nsga3_alg = RNSGA3(ref_points=ref_points, pop_per_ref_point=m+6, mu=0.1)
...
if res_gp_mo.X is not None:
    ...
    scipy.io.savemat('three_obj_hce_result_20cp_200ps_20os_'+str(max_no_subgraphs)+'_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.mat',results)
...
plt.yscale("log")
plt.savefig('three_obj_hce_result_20cp_200ps_20os_'+str(max_no_subgraphs)+'_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.png',dpi=75)
plt.show()
