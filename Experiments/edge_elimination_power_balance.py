...
class GraphPartition(Problem):
    ...
    def __init__(self, _G, adj_matrix, xcoord, ycoord, _lode_info,_gen_info,partition_size_upper = 20, partition_size_lower=0, min_nodes_partition = 3):
        self.nodes = len(adj_matrix)
        self.adj = adj_matrix
        self.x_loc = xcoord
        self.y_loc = ycoord
        self.lode_info = _lode_info
        self.gen_info = _gen_info
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        # fifth cost is to minimize the difference in load and gen real power within a partition
        f5=[]
        res_f5={}

        for d in range(x.shape[0]):
            threads_f2[d] = threading.Thread(target=self.compute_metric_second_third_fourth_fifth_objective, args=(x[d],res_f2,res_f3,res_f4,res_f5, d))
            threads_f2[d].start()
        for d in range(x.shape[0]):
            threads_f2[d].join()
        ...
        out["F"] = anp.column_stack([np.array(f1), np.array(f4), np.array(f5)])
    ...
    def compute_metric_second_third_fourth_fifth_objective(self,sol,res,res2,res3,res4,ix):
        edge_index = np.where(sol == 1)
        type_node = nx.get_node_attributes(self.G, 'nodetype')
        weight_label = nx.get_edge_attributes(self.G, 'pflow')
        net_cost = 0
        ...
        for i, si in enumerate(subgraph):
            for node in list(si.nodes):
                if node in gen_info.keys():
                    if i not in gen_partition_wise.keys():
                        gen_partition_wise[i] = self.gen_info[node][1]
                    else:
                        gen_partition_wise[i] += self.gen_info[node][1]
        for i, si in enumerate(subgraph):
            if i in gen_partition_wise.keys() and i in load_partition_wise.keys():
                net_cost_4+=(load_partition_wise[i] + gen_partition_wise[i])
            elif i in load_partition_wise.keys():
                net_cost_4+=load_partition_wise[i]
        res4[ix] = net_cost_4
...
power_file=r'C:\Users\asahu\Desktop\Proj2_REORG\Experiments\PYMOO\pymoo\pymoo\usage\Notebooks\Snowmass_D1_Sub_D1__D7_11_D8__EXP_ElemPowers_Mod.CSV'
sd = SystemDescription(_power_file=power_file,_case='hce_mod')
...
if res_gp_mo.X is not None:
    ...
    plt.savefig('3P_obj_hce_grid_'+str(cp)+'cp_'+str(ps)+'ps_'+str(os)+'os_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.png', dpi=600)
else:
...
if save_soln_history:
    ...
    plt.yscale("log")
    plt.savefig('3P_obj_hce_result_'+str(cp)+'cp_'+str(ps)+'ps_'+str(os)+'os_'+str(max_no_subgraphs)+'_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.png',dpi=75)
    plt.show()
else:
