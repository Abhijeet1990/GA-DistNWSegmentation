...
class GraphPartition(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        # less than 1 and greater than 1 means equals 1
        for i in range(self.nodes):
            g_temp = np.sum(x[:,self.zones*i : self.zones*(i+1)],axis=1) - 1
            g.append(g_temp)
...
sd = SystemDescription(_case='ieee13_der')
...
if res_gp_mo.X is not None:
    ...
    scipy.io.savemat('new_case_all_f_ieee13bus_der_constraint_changed_result_'+str(zones)+'_'+str(max_nodes_per_zone)+'.mat',results)
