...
def get_input_res(bus_graph, active_nodes, node_dict, gen_nodes, gen_node_cap, diesel_left,
    ...
    return min_feasible_subnetworks
...
class GraphPartition(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        res_r2={}
        for d in range(x.shape[0]):
            threads_f5[d] = threading.Thread(target=self.compute_resilience_metric, args=(x[d],res_r1, d))
            threads_f5[d].start()
        for d in range(x.shape[0]):
            threads_f5[d].join()
        ...
        out["F"] = anp.column_stack([ np.array(f2), np.array(f3), np.array(f5)])
    ...
    def compute_resilience_metric(self, sol, res_1r, ix):
        temp_graph = self.G.copy()
        edge_index = np.where(sol == 1)
        type_node = nx.get_node_attributes(self.G, 'nodetype')
        # get the edge index from original graph
        ...
        #path_redundancy, \
        min_feasible_subnetworks = get_input_res(**input_dict)
        #res_1r[ix] = path_redundancy
        res_1r[ix] = min_feasible_subnetworks
