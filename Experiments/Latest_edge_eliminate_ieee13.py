...
class GraphPartition(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        #out["F"] = anp.column_stack([np.array(f4)])
...
if res_gp_mo.X is not None:
    ...
    for k, subgraph in enumerate(S):
        for node in subgraph.nodes:
            nx.draw_networkx_nodes(G, pos, [node], node_size=72, node_color=colorlist[k])
    ...
    plt.savefig('four_obj_ieee34_grid_'+str(cp)+'cp_'+str(ps)+'ps_'+str(os)+'os_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.png', dpi=600)
else:
...
if save_soln_history:
    ...
    plt.yscale("log")
    plt.savefig('four_obj_ieee34_result_'+str(cp)+'cp_'+str(ps)+'ps_'+str(os)+'os_'+str(max_no_subgraphs)+'_'+str(min_no_subgraphs)+'_'+str(min_node_partition)+'.png',dpi=75)
    plt.show()
else:
