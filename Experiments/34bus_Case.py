...
for p in range(4,10):
    for m in range(25,30):
        results = {}
        gp_problem = GraphPartition(G, adj_matrix=Adj, partitions=3,partition_size_upper=35,xcoord=np.array(xcoords), ycoord=np.array(ycoords))
        ...
        if res_gp_mo.X is not None:
            ...
            scipy.io.savemat('nsga_ieee34bus_result_'+str(p)+'_'+str(m)+'.mat',results)
