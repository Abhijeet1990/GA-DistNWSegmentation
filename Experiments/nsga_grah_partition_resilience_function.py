...
def get_path_redundancy(bus_graph_final, gen_nodes, gen_nodes_new, gen_node_cap, diesel_left, coeff_b, time_step,
    ...
    for load_bus in np.arange(len(load_nodes)):
        ...
        for gen_bus in np.arange(len(gen_nodes_new)):
            ...
            #print('shortest path load to gen '+str(_path))
        ...
        for load_bus_1 in np.arange(len(load_nodes)):
            ...
            #print('shortest path load to load ' + str(_path))
            if _path is not None:
    ...
    print('Path redundancy val '+str(path_redundancy_val))
    path_redundancy_val = path_redundancy_val / (len(load_nodes) * (len(gen_nodes)+len(load_nodes)))
    return path_redundancy_val

def get_min_feasible_subnetworks(bus_graph_final, gen_nodes_new, gen_node_cap, p_cr, diesel_left, coeff_b, time_step, node_dict, ess_left, bat_p_cap,active_nodes):
    #load_nodes = list(bus_graph_final.keys())
    #load_nodes.remove(0)
    load_nodes = active_nodes
    if 0 in load_nodes:
    ...
    for cnt in np.arange(subnetwork_cnt + 1):
        ...
        for _bus in subnetwork_dict[cnt]:
            if (_bus in gen_nodes_new) and (_bus not in load_nodes) and (_bus in gen_dict.keys()) and (_bus in ess_p_dict.keys()):
                net_gen += gen_dict[_bus] + ess_p_dict[_bus]
                # print(" +: ", net_gen)
            elif (_bus not in gen_nodes_new) and (_bus in load_nodes) and (_bus in load_dict.keys()) and (_bus in ess_p_dict.keys()):
                net_gen += -load_dict[_bus] + ess_p_dict[_bus]
                # print(" -: ", net_gen)
            elif (_bus in gen_nodes_new) and (_bus in load_nodes) and (_bus in load_dict.keys()) and (_bus in ess_p_dict.keys()) and (_bus in gen_dict.keys()):
...
def get_input_res(bus_graph, active_nodes, node_dict, gen_nodes, gen_node_cap, diesel_left,
    ...
    min_feasible_subnetworks = get_min_feasible_subnetworks(bus_graph, gen_nodes_new, gen_node_cap, p_cr[:, 0] + p_pv[:, 0], diesel_left, coeff_b, time_step, node_dict, ess_left, bat_p_cap, active_nodes)
...
class GraphPartition(Problem):
    ...
    def compute_resilience_metric(self, sol, res_1r, res_2r, ix):
        ...
        active_nodes = ['1_a', '2_b', '4_c', '5_c', '6_c', '7_a', '9_a', '10_a', '11_a', '12_b', '16_c', '17_c', '19_a',
                        '20_a','22_b', '24_c', '28_a', '29_a', '30_c', '31_c', '32_c', '33_a', '34_c', '35_a', '37_a', '38_b',
                        '39_b','41_c', '42_a', '43_b', '45_a', '46_a', '47_a', '48_a', '49_a', '50_c', '51_a', '52_a', '53_a',
                        '55_a','56_b', '58_b', '59_b', '60_a', '62_c', '63_a', '64_b', '65_a', '66_c', '68_a', '69_a', '70_a',
                        '71_a','73_c', '74_c', '75_c', '76_a', '77_b', '79_a', '80_b', '82_a', '83_c', '84_c', '85_c', '86_b',
                        '87_b','88_a', '90_b', '92_c', '94_a', '95_b', '96_b', '98_a', '99_b', '100_c', '102_c', '103_c',
                        '104_c','106_b', '107_b', '109_a', '111_a', '112_a', '113_a', '114_a']  # gens+loads
