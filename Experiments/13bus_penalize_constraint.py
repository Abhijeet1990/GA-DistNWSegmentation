    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for d in range(x.shape[0]):
            ...
            for j in range(self.zones):
                ...
                penalties+= 1000*(shortest_paths_within_zone(self.G, sol, element_ix, j, self.zones) - M)
            f4.append(penalties)

        #out["F"] = anp.column_stack([np.array(f1), np.array(f2), np.array(f3)])
        out["F"] = anp.column_stack([np.array(f1), np.array(f3), np.array(f4)])
...
results = {}
gp_problem = GraphPartition(G, adj_matrix=Adj, partitions=zones,partition_size_upper=max_nodes_per_zone,partition_size_lower=0,xcoord=np.array(xcoords), ycoord=np.array(ycoords))
...
if res_gp_mo.X is not None:
    ...
    scipy.io.savemat('all_ieee13bus_resultv2.mat',results)
